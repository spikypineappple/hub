...
## Alternative — Replicate rembg for transparent output (2026-05-21)

When the subject is complex (people, hair, soft edges) or you don't want to corrupt the
generation prompt with chroma-key instructions, use Replicate's bg-remover as a second
pass instead. **Importantly, if you ask Gemini for a "transparent background" directly,
it bakes a flat checkerboard pattern as RGB pixels — verify with `magick identify` that
the output is actually `srgba` (4 channels), not `srgb` (3 channels).**

Pipeline:
1. Generate the subject normally (with the lighting/styling you want)
2. Upload the output to a public URL (e.g. GCS bucket)
3. Call `cjwbw/rembg` on Replicate — version `fb8af171cfa1616ddcf1242c093f9c46bcada5ad4cf6f2fbe8b81b330ec5c003`
...
curl -s -X POST "https://api.replicate.com/v1/predictions" \
  ...
  -H "Prefer: wait=60" \
  -d "{\"version\":\"fb8af171...\",\"input\":{\"image\":\"$URL\"}}"
```

Returns a true RGBA PNG with proper alpha. For tougher edges (long hair, fur) try
`men1scus/birefnet` instead.

**Replicate API note**: `Prefer: wait=` clamps at 60 max — passing 90 errors with
`"must specify a value between 1 and 60"`. Poll the prediction URL for longer runs.

## Face-preserving restyle pipeline (2026-05-21)

When restyling a real person's photo (new background, new clothes, new lighting),
do NOT prompt the multi-image model to "change X and keep the face". It will morph
the face every time, especially when you ask for any beard / hair / age tweak.

Instead, split the transformation:
1. Use nano-banana for one tight change at a time ("only swap the shirt to a black tee,
   keep everything else identical") — works well because the scope is narrow
2. Run the output through `cjwbw/rembg` to extract just the person on alpha
3. Composite the cutout onto a freshly-rendered HTML/CSS background via ImageMagick

```bash
magick background.png cutout.png -gravity center -composite final.png
```

This preserves the source face exactly because the bg pass never touches pixels inside
the alpha mask. Costs ~$0.04 per round (nano-banana + rembg) and the result holds up.

If James says "doesn't look like me", the face is being morphed by an over-ambitious
prompt — drop scope and ask for ONE change at a time, or skip the model entirely and
do the swap via cutout + composite.
